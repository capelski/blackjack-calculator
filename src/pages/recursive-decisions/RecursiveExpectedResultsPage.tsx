...
import {
  ...
  calculatePlayerRoi,
  outcomeClass,
} from '../expected-results/expectedResultsLogic'
import {
...
function groupScoresByFinalLabel(
  combinations: Array<{ score: string; probability: number }>,
): Map<string, number> {
...
function RecursiveExpectedResultsPage() {
  ...
  const playerScores = useMemo(
    () => groupScoresByFinalLabel(collectRecursiveFinalCombinations(treePolicy)),
    [treePolicy],
  )
  ...
  return (
    <main className="combination-page">
      <header className="combination-header">
        ...
        <p className="intro">
          Compare the recursive-policy player distribution against the dealer distribution
          (dealer threshold fixed at 17).
        </p>
      </header>
...
export default RecursiveExpectedResultsPage
